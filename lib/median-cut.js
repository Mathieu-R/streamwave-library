...
function main (path) {
  ...
  promisify(getPixels)(path)
    .then(pixels => pixelsRGB(pixels))
    .then(pixels => quantize(pixels, 0))
    .then(quantizedPixels => {
      console.log(quantizedPixels);
      return sortByLuminance(quantizedPixels)[0];
    })
...
function quantize(pixels, depth) {
  // beginning
  if (depth === 0) {
    console.log("Quantizing to %d buckets.", Math.pow(2, MAX_DEPTH))
  }
  ...
  return [
    ...
    ...quantize(pixels.slice(middle), depth + 1)
  ];
}
